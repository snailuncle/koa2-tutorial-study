...
function 悬浮控制(window, windowid, ar, view) {
    ...
    this.orientationInterval = setInterval(() => {
        if (context.resources.configuration.orientation != this.Orientation) {
            ...
            if(view){
ui.run(()=>{
    view.attr("w",Math.floor(this.Width*0.8));
    
      /*          
params = view.getLayoutParams(); 
params.width = Math.floor(this.Width*0.8); 
params.height = Math.floor(this.Height*0.8); 
