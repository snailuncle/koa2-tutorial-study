    ...
    canvasView.on("draw", (canvas) => {
        ...
        canvas.drawText(String("B"), x, y, this.paint);
            ...
            if (this.TouchResult.length) {
                ...
                var X = this.TouchPointCurrent[(i-1)*2];
                var Y = this.TouchPointCurrent[(i-1)*2 + 1];
                X = X || 0;
                Y = Y || 0;
            ...
            switch (event.getActionMasked()) {
                case event.ACTION_MOVE:
                    for (let i = 0; i < PC; i++) {
                        ...
                        let BX = this.TouchPointCurrent[(i-1)*2]||AX;
                        let BY = this.TouchPointCurrent[(i-1)*2+1]||AY;
                        let CX = X;
                        let CY = Y;

                        let S = this.weiyi([AX - BX, AY - BY]);
                        let S0 = this.VC * S;
                        let S1 = this.V1S * S;
                        let S2 = this.V2S * S;
                        let R = (S0 + S1) / 2;
                        let OB = R + S2;
                        let O_ary = this.getsd(OB, [AX - BX, AY - BY]);
                        let OX = BX + O_ary[0];
                        let OY = BY + O_ary[1];
                        let AB_ary = this.XYTOAB(AX, AY, CX, CY);
                        this.TouchResult[i] = {
                            ox: OX,
                            oy: OY,
                            r: R,
                            pos: this.SolvePos(OX, OY, R, AB_ary[0], AB_ary[1])
                        };
                    };
